  ...
  <?php
    ...
    $input = $_POST['input'];
		...
		$qry = mysqli_query($con, "SELECT  equipment.equip_name AS equip_name,
					...
					WHERE stat = 'pending' AND stat != 'done' AND student.id_number = '$input' ")or die(mysqli_error($con));
           ...
           while($row = mysqli_fetch_array($qry)){ ?>
                <tbody>
                    ...
                    <td><?php echo $row['uid'] ?></td>
                        ...
                        <td><a href="admin_app.php?req_id=<?php echo $row['req_id']?>"><button class='btn btn-info'>Approved</button></a> <a href='delete_sched.php?sched_id=$row[9]'><button class='btn btn-danger'>Delete</button></a></td>
                    </tr>
                </tbody>
    <?php   }
        } 
